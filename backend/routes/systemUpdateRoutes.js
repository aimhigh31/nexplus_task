...
router.get('/', async (req, res) => {
  try {
    ...
    console.log('시스템 업데이트 조회 필터:', JSON.stringify(filter));
    
    // 데이터 조회 및 응답
    const updates = await SystemUpdate.find(filter).sort({ no: -1 });
    res.json(updates);
  } catch (error) {
    console.error('시스템 업데이트 조회 오류:', error);
    res.status(500).json({ message: '시스템 업데이트 데이터 조회 중 오류가 발생했습니다.', error: error.message });
  }
});

// 특정 시스템 업데이트 조회 (번호 기준)
router.get('/:id', async (req, res) => {
  try {
    const update = await SystemUpdate.findOne({ no: req.params.id });
    
    if (!update) {
      return res.status(404).json({ message: '해당 번호의 시스템 업데이트를 찾을 수 없습니다.' });
    }
    
    res.json(update);
  } catch (error) {
    console.error('시스템 업데이트 상세 조회 오류:', error);
    res.status(500).json({ message: '시스템 업데이트 상세 조회 중 오류가 발생했습니다.', error: error.message });
  }
});

// 특정 시스템 업데이트 조회 (코드 기준)
router.get('/code/:code', async (req, res) => {
  try {
    ...
    console.error('시스템 업데이트 코드 조회 오류:', error);
    res.status(500).json({ message: '시스템 업데이트 코드 조회 중 오류가 발생했습니다.', error: error.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    console.error('시스템 업데이트 추가 오류:', error);
    ...
    res.status(500).json({ message: '시스템 업데이트 추가 중 오류가 발생했습니다.', error: error.message });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    console.error('시스템 업데이트 수정 오류:', error);
    res.status(500).json({ message: '시스템 업데이트 수정 중 오류가 발생했습니다.', error: error.message });
  }
});
...
router.put('/code/:code', async (req, res) => {
  try {
    ...
    console.error('시스템 업데이트 코드 수정 오류:', error);
    res.status(500).json({ message: '시스템 업데이트 코드 수정 중 오류가 발생했습니다.', error: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.json({ message: '시스템 업데이트가 성공적으로 삭제되었습니다.', deletedUpdate: update });
  } catch (error) {
    console.error('시스템 업데이트 삭제 오류:', error);
    res.status(500).json({ message: '시스템 업데이트 삭제 중 오류가 발생했습니다.', error: error.message });
  }
});
...
router.delete('/code/:code', async (req, res) => {
  try {
    ...
    console.error('시스템 업데이트 코드 삭제 오류:', error);
    res.status(500).json({ message: '시스템 업데이트 코드 삭제 중 오류가 발생했습니다.', error: error.message });
  }
});
